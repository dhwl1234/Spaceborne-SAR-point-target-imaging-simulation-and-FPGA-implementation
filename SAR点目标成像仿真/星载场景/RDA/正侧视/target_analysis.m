function [PSLR_r,ISLR_r,IRW_r, PSLR_a,ISLR_a,IRW_a] = target_analysis(s_ac,Fr,Fa,Vr,L_range,L_azimuth)
% 这是对应多普勒处理方式2：
% 采用“移动频率轴”时的对应函数
...
[aa,p] = max(abs(s_ac));           %max寻找每列最大的数存在aa里，所在行数存在p里
[bb,q] = max(max(abs(s_ac)));%%bb是abs(s_ac)中的最大值
...
%%
% 下面进行二维升采样
S_ac_test_1 = fft(s_ac_test,[],1);     % 方位向fft,1是column列方向
S_ac_test_2 = fft(S_ac_test_1,[],2);   % 距离向fft，2是row行方向
...
% 作图
figure('NumberTitle', 'off', 'Name', '升采样之后作图');
subplot(1,2,1);
...
S_AC_test_row_max(1,16*length(S_AC_test_row_max_1)+1-(length(S_AC_test_row_max_1)-I1):end) ...
    = S_AC_test_row_max_1(1,I1+1:end);
% 完成16倍升采样     
...
S_AC_test_column_max(1,16*length(S_AC_test_column_max_1)+1-(length(S_AC_test_column_max_1)-I2):end)...
    = S_AC_test_column_max_1(1,I2+1:end);
% 完成16倍升采样   
% ====================================================================
...
%%
% 作图
figure('NumberTitle', 'off', 'Name', '升采样之后16倍作图');
subplot(1,2,1);
...
%%
% %距离向相位
figure('NumberTitle', 'off', 'Name', '切片相位');
subplot(1,2,1);
plot(angle(s_ac_test_row_max));
title('距离向相位');
 grid on;
subplot(1,2,2);
plot(angle(s_ac_test_column_max));
title('方位向相位');
 grid on;
%%
% 行切片，点目标中心距离向指标
...
IRW_XISHU_r=IRW_r/L_range;
parameter_r = [PSLR_r,ISLR_r,IRW_r,IRW_XISHU_r];

% 列切片，点目标中心方位向指标
%慢时间域
[PSLR_a,ISLR_a,IRW_a] = zhibiao_2(s_ac_test_column_max,16*8*NN,NN/Fa);
IRW_a = (IRW_a/c*2)*Vr;
IRW_XISHU_a=IRW_a/L_azimuth;
parameter_a = [PSLR_a,ISLR_a,IRW_a,IRW_XISHU_a];


disp('------------------------------------------------------');
disp('行切片，点目标中心距离向指标');
disp('      PSLR     ISLR       IRW         展宽系数');
disp(parameter_r);
disp('------------------------------------------------------');
disp('列切片，点目标中心方位向指标');
disp('      PSLR     ISLR       IRW         展宽系数');
