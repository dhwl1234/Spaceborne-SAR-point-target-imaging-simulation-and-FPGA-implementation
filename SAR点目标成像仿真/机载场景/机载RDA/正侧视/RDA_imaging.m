...
sita_r_c = (0*pi)/180;	% 波束斜视角，3.5 度，这里转换为弧度
c = 3e8;                    % 光速
...
% 作图
% 图1——原始数据
figure('NumberTitle', 'off', 'Name', '机载正侧视对原始数据进行绘图');
subplot(2,2,1);
% mesh(real(s_echo));
 imagesc(real(s_echo));
% plot(real(s_echo));
...
xlabel('距离时域（采样点）');
ylabel('方位时域（采样点）');
text(300,-40,'机载正侧视原始数据');       % 给图1进行文字说明 
...
figure('NumberTitle', 'off', 'Name', '机载正侧视对原始数据变换至距离多普勒域和二维频域之后进行绘图');

subplot(2,2,1);
imagesc(abs(fft(s_echo,[],1)));%%
title('RD 频谱幅度');
text(200,-40,'机载正侧视原始数据变化至距离多普勒域和二维频域');       % 给图2进行文字说明 
subplot(2,2,2);
...
figure('NumberTitle', 'off', 'Name', '机载正侧视对原始数据变换至距离维频域之后');
...
ylabel('方位时域（采样点）');
text(340,-10,'未压缩');       
text(200,-40,'机载正侧视原始数据变化至离维频域的情况');       % 给图2进行文字说明 
subplot(1,2,2);
% mesh(abs(S_range));
imagesc(abs(S_range));
title('（b）幅度');
...
xlabel('距离频域（采样点）');
ylabel('方位时域（采样点）');
text(280,-60,'机载正侧视距离频域');       % 给图3进行文字说明
text(340,-10,'已压缩');       
...
xlabel('距离时域（采样点）');
ylabel('方位时域（采样点）');
text(150,-60,'机载正侧视二维时域');       % 给图4进行文字说明
text(172,-10,'完成压缩');       
...
delta_Rrd_fn = lamda^2.*((fa.').^2)*(R0_RCMC)/(8*Vr^2);%求出需要校正的距离徙动
...
xlabel('距离时域（采样点）');
ylabel('方位频域（采样点）');
text(150,-60,'机载正侧视距离多普勒域');       % 给图5进行文字说明
text(172,-10,'未RCMC');       
subplot(1,2,2);
imagesc(abs(S_rd));
title('（b）幅度');
...
xlabel('距离时域（采样点）');
ylabel('方位频域（采样点）');
text(150,-60,'机载正侧视距离多普勒域');       % 给图6进行文字说明
text(172,-10,'已RCMC');       
...
Ka = 2*Vr^2*(cos(sita_r_c))^3./(lamda.* R0_RCMC);  	% 方位向调频率，是随最近斜距R0变化的。书本P86
