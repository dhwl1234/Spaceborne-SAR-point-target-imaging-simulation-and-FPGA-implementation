...
ISLR = 10*log10((sum(y(1,P1:P1))+sum(y(1,P2:end)))/sum(y(1,P1:P2)));% 积分旁瓣比。
%%
%%%%%%%%%%%%%%%%%%%%%%%  以下是求 IRW  %%%%%%%%%%%%%%%%%%%%%%%%%
...
% 计算左边3dB的那个坐标
if soo_abs(Th_x1)-M < 0%判断3db点在距离最近点的左边还是右边
    x0_linear = Th_x1;
    x1_linear = Th_x1+1;
else
    x0_linear = Th_x1-1;
 ...
 Th_x1_real = (( x1_linear - x0_linear )/(soo_abs(x1_linear)-soo_abs(x0_linear)))*...
(M-soo_abs(x0_linear))+x0_linear;
...
if soo_abs(Th_x2)-M > 0
 ...
 Th_x2_real = (( x1_linear - x0_linear )/(soo_abs(x1_linear)-soo_abs(x0_linear)))*...
(M-soo_abs(x0_linear))+x0_linear;
% ------------------------------------------------------------------
