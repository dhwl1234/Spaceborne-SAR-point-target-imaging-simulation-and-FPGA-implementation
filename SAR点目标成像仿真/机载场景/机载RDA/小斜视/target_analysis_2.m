function [PSLR_r,ISLR_r,IRW_r, PSLR_a,ISLR_a,IRW_a] = target_analysis_2(s_ac,Fr,Fa,Vr,L_range,L_azimuth)
...
[aa_test,p_test] = max(abs(s_ac_test));           %最大值所在行数存在P里，aa是每一列的最大值
[bb_test,q_test] = max(max(abs(s_ac_test)));%bb是数组中的最大值，q是所在的列数
...
[aa_test_2,p_test_2] = max(abs(s_ac_test(1:row_test_max,1:2*column_test_max/3)));    %求每列中最大值所在行数       
[bb_test_2,q_test_2] = max(max(abs(s_ac_test(1:row_test_max,1:2*column_test_max/3))));%求最大值所在列数
...
% 作图
figure('NumberTitle', 'off', 'Name', '将距离向旁瓣旋转到平行于水平轴后的成像结果');
...
% s_ac_test_row_max = s_ac_range(row_test_max_range,:);
% 保留全部
