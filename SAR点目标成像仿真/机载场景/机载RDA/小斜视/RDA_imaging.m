...
Ls = 0.886*R_nc*lamda/La_real;              % 合成孔径长度
...
% 作图
% 图1——原始数据
figure('NumberTitle', 'off', 'Name',  '机载小斜视对原始数据进行绘图');
subplot(2,2,1);
imagesc(real(s_echo));
title('（a）实部');
xlabel('距离时域（采样点）');
ylabel('方位时域（采样点）');
text(300,-40,'机载小斜视原始数据');       % 给图1进行文字说明 
...
figure('NumberTitle', 'off', 'Name', '机载小斜视对原始数据变换至距离多普勒域和二维频域之后进行绘图');
subplot(2,2,1);
...
xlabel('距离频域（采样点）');
ylabel('方位时域（采样点）');
text(280,-60,'机载小斜视距离频域');       % 给图3进行文字说明
text(340,-10,'已压缩');       
